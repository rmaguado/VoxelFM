...
class VolumeDataset(Dataset):
    def __init__(
        ...
        self.normalize = transforms.Normalize(fmean, fstd, inplace=True)

    def __len__(self) -> int:
        return len(self.uids)

    def _load_volume(self, uid: str) -> torch.Tensor:
        path = os.path.join(self.volumes_path, f"{uid}.pth")
    ...
    def __getitem__(self, idx: int):
        x = self._load_volume(self.uids[idx])
        y = torch.tensor(self.labels[idx], dtype=torch.float32)
